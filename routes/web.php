...
Route::middleware('auth')->group(function () {
    ...
    // CATÁLOGO DE PRODUCTOS DEL PROVEEDOR (accesible por proveedores autenticados)
    Route::middleware('role:proveedor')->group(function () {
        Route::resource('proveedor-productos', ProveedorProductoController::class)
            ->except(['show']);
        Route::post('proveedor-productos/{proveedorProducto}/toggle-disponible', [ProveedorProductoController::class, 'toggleDisponible'])
            ->name('proveedor-productos.toggle-disponible');
    });
    ...
    Route::middleware('role:ankor_user')->group(function () {
        ...
        Route::post('clientes/{cliente}/toggle-activo', [ClienteController::class, 'toggleActivo'])->name('clientes.toggle-activo');

        // SOLICITUDES DE COTIZACIÓN
        Route::resource('solicitudes-presupuesto', SolicitudPresupuestoController::class)
            ->parameters(['solicitudes-presupuesto' => 'solicitud'])
            ->except(['edit', 'update']);
        ...
        Route::resource('pedidos-cliente', PedidoClienteController::class)
            ->parameters(['pedidos-cliente' => 'pedidoCliente']);
        ...
        Route::post('pedidos-cliente/{pedido}/solicitar-todos', [PedidoClienteController::class, 'solicitarCotizacionTodos'])->name('pedidos-cliente.solicitar-todos');
        ...
        // ÓRDENES DE ENVÍO
        Route::resource('ordenes-envio', OrdenEnvioController::class)
            ->parameters(['ordenes-envio' => 'ordenEnvio'])
            ->except(['edit', 'update']);
        ...
        // EXPORTACIÓN PDF
        Route::get('pdf/pedido/{pedido}', [PdfController::class, 'pedidoCliente'])->name('pdf.pedido-cliente');
        Route::get('pdf/orden-compra/{orden}', [PdfController::class, 'ordenCompra'])->name('pdf.orden-compra');
        Route::get('pdf/orden-envio/{orden}', [PdfController::class, 'ordenEnvio'])->name('pdf.orden-envio');
        Route::get('pdf/solicitud/{solicitud}', [PdfController::class, 'solicitudPresupuesto'])->name('pdf.solicitud-presupuesto');

        // ANÁLISIS DE PROVEEDORES
        Route::prefix('analisis-proveedores')->name('analisis-proveedores.')->group(function () {
            Route::get('/', [AnalisisProveedoresController::class, 'index'])->name('index');
            Route::get('/productos', [AnalisisProveedoresController::class, 'productos'])->name('productos');
            Route::get('/producto/{producto}', [AnalisisProveedoresController::class, 'producto'])->name('producto');
            Route::get('/proveedor/{proveedor}', [AnalisisProveedoresController::class, 'proveedor'])->name('proveedor');
            Route::post('/actualizar-precios', [AnalisisProveedoresController::class, 'actualizarPrecios'])->name('actualizar-precios');
        });
    });
});
